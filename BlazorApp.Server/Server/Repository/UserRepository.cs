...
namespace BlazorApp.Repository
{
    public class UserRepository : IUser
    {
        ...
        public async Task<UserResponse> Register(RegisterData registerData)
        {
            try
            {
                UserData user = new()
                ...
                await _context.Deliveryaddress.AddAsync(deliveryAddress);
        ...
        public async Task<UserResponse> UpdateAddress(UserData addressOld, DeliveryAddress newAddress)
        {
            try
            {
                ...
                var delivery = await _context.Deliveryaddress
        ...
        public UserResponse DeleteUser(Guid id)
        {
            try
            {
                ...
                {
                    _context.Deliveryaddress.Remove(user.Delivery);
                }
        ...
        public async Task<UserResponse> AuthenticateUser(LoginModel data)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(data.Credential) || string.IsNullOrWhiteSpace(data.Password))
                {
                ...
                var user = await _context.Users.SingleOrDefaultAsync(u => u.Username == data.Credential && u.Password == data.Password);

                if (user == null)
                {
                    user = await _context.Users.SingleOrDefaultAsync(u => u.Email == data.Credential && u.Password == data.Password);
                    if (user == null)
                    {
                        return new UserResponse { Message = "Username or password is invalid", Status = false };
                    }
        ...
        public Role GetUserRole(Guid Id)
        {
            try
            {
            ...
            {
                // Логирование ошибки
                _logger.LogError(ex, $"An error occurred while retrieving the role for user with ID: {Id}");
        ...
        public UserResponse GetUserData(LoginModel data)
        {
            try
            {
